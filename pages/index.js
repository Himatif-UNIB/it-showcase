...
import {
  ...
  CategoryLists,
  UserLists,
} from "../components/ShowcasesPage/Sidebar";
import FilterContext from "../helpers/filter/FilterContext";
import { useRouter } from "next/router";
import Close from "../components/Icons/Close";
import axios from "../config";

export default function Home({ initialCategories, initialShowcases, initialContri }) {
  ...
  useEffect(() => {
    ...
    router.push(
      `${pathname}?search=${searchQuery}&categories=${selectedCategories}&type=${selectedType}`
    );
  ...
  return (
    <Layout>
      ...
      <section className="mt-12 px-6">
        <>
          <Searchbar
            toggleExpandedFilter={() => setExpandedFilter(!expandedFilter)}
          />
          <div className="mt-5 pb-20 md:grid md:grid-cols-4 md:gap-6 md:pb-0">
            <aside
              className={
                (expandedFilter
                  ? "md:col-span-0 translate-y-0 md:hidden"
                  : "translate-y-[100%] md:col-span-1 md:translate-y-0") +
            ...
            >
              ...
              <UserLists initialContri={initialContri} />
              <TypeLists />
              <CategoryLists initialCategories={initialCategories} />
              <SmallButtonAction
                handleClearAllFilters={filterContext.handleClearAllFilters}
...
export async function getServerSideProps({ query }) {
  ...
  const contri = await axios.get("/contributor");
  const initialContri = contri.data;
  const initialShowcases = await getShowcasesData(
    query.categories,
    query.search,
    query.type
  );
  console.log("Initial Categories ", contri);
  ...
  return (
    ...
    >
      <div className="flex justify-center">
        <button
          className="rounded-xl bg-blue-500 px-36 py-3 font-semibold text-white"
...
function SmallButtonAction({ handleClearAllFilters, setExpandedFilter }) {
  return (
    <div className="sticky bottom-0 block w-full border-t border-dashed border-gray-500 bg-dark py-4 ">
      <div className="grid grid-cols-2 gap-3">
        ...
        <button
          className="rounded-xl bg-blue-500 px-6 py-3 text-lg font-medium"
